#!/usr/bin/env python3
"""
Chainer - Core logic penggabungan semua scanner
"""
...
class ScanChainer:
    def __init__(self):
        # Hapus hasil lama dan buat folder baru
        self.output_dir = create_output_dirs(
            config.get("reporting.output_dir", "output"), 
            clean=True
        )
        console.print(f"[cyan]Output directory dibersihkan: {self.output_dir.resolve()}[/cyan]")
        ...
        self.max_workers = config.get("scan.max_parallel_workers", 8)
    ...
    def run_full_scan(self, target: str, enable_os: bool = False) -> None:
        ...
        try:
            ...
            # Tahap 5: ffuf Directory Brute-Force
            if config.get("ffuf.enabled", True):
            ...
            console.print(f"[bold]Semua hasil tersimpan di folder:[/bold] {self.output_dir.resolve()}")
    ...
    async def _run_parallel_nikto(self, web_targets: list):
        """Jalankan Nikto secara parallel"""
        if config.get("evasion.enabled", False):
            print_stealth_info()

        tasks = [
            self.nikto_scanner.scan_async(
                web['url'], 
